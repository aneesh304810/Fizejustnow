...
export const api = {
  ...
  legacyLineageTables: () => get('/legacy-lineage/tables', () => ({ tables: [] })),
  legacyLineageGroups: () => get('/legacy-lineage/groups', () => ({ groups: [] })),
  legacyDependencyNetwork: (o) =>
    get(`/legacy-lineage/dependency-network?include_excluded=${o && o.include_excluded ? 'true' : 'false'}`,
      () => ({ edges: [], nodes: [] })),
  legacyLineageFields: (table) => get(`/legacy-lineage/fields?table=${encodeURIComponent(table)}`, () => ({ fields: [] })),
  ...
  legacyBusinessDef: (code, system) =>
    get(`/legacy-lineage/business-def?code=${encodeURIComponent(code)}${system ? `&system=${encodeURIComponent(system)}` : ''}`,
      () => ({ definition: null })),
